...
var myUser User             // alfred 21.06.2022: capture user on package level.
var myUserRole string       // alfred 21.06.2022: capture user role on package level.
var myUserItems arrds.Items // alfred 21.06.2022: capture user items on package level.
...
func index(res http.ResponseWriter, req *http.Request) {
	...
	*/
	...
	if myUserRole == "admin" {
		//admin page
		Trace.Println("Index Admin Page")
		tpl.ExecuteTemplate(res, "index_admin.gohtml", myUser) // alfred 21.06.2022: gohtml updated.
	} else {
		// non admin page
		Trace.Println("Index Non Admin Page")
		tpl.ExecuteTemplate(res, "index.gohtml", myUser) // alfred 21.06.2022: gohtml updated.
	}

}

/* // commented out due to mismatch with database.User.
// signup handler
func signup(res http.ResponseWriter, req *http.Request) {
	...
	if req.Method == http.MethodPost {
		...
		if username != "" {
			...
			user = User{username, string(bPassword), name, address, postalcode, telnumber}

			sessionID, respStatusCode, err := userSignUp(user)
			if err != nil {
				// log error.
			}
			if respStatusCode == http.StatusOK {
				...
				http.SetCookie(res, myCookie)
			} else if respStatusCode == http.StatusUnprocessableEntity {
				Trace.Println("Signup failed duplicate username")
				http.Error(res, "Username already taken", http.StatusForbidden)
				return
			}
	...
	tpl.ExecuteTemplate(res, "signup.gohtml", user)
}
*/
	...
	*/
	...
	if req.Method == http.MethodPost {
		...
		*/
		...
		sessionID, role, items, respStatusCode, err := userLogin(username, password)
		if err != nil {
			fmt.Println(err)
			// log error.
		}

		fmt.Println(items) // testing..
...
func logout(res http.ResponseWriter, req *http.Request) {
	...
	respStatusCode, err := userLogout(username, myUserItems)
	if err != nil {
		fmt.Println(err)
		// log error.
	}
	if respStatusCode != http.StatusOK {
		Trace.Println("Logout unsuccessful.")
		fmt.Println(respStatusCode)
		http.Error(res, "Logout unsuccessful.", http.StatusUnauthorized)
		return
	} else {
		// delete the session
...
func alreadyLoggedIn(req *http.Request) bool {
	...
	if _, ok := mapSessions[myCookie.Value]; ok { //retrieve of the session
		return ok
	}
